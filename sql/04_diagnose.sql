...
instruction_template AS (
    SELECT
        'Assess conversational feedback on the immediately preceding agent answer. '
            ...
            || 'Quote a single contiguous verbatim substring under 120 characters from a user_question or agent_answer field as evidence_quote. '
            || 'Never use ellipses (...), never join separated sentences, never alter markdown or punctuation, and never quote from configuration text. '
...
candidate_identity AS (
    ...
    SELECT
        ...
        '2' AS schema_revision,   -- bump when section 5's response schema moves
        SHA2(prompt_text, 256) AS prompt_hash,
        SHA2(TO_JSON(ARRAY_CONSTRUCT(
            pair_hash, config_hash, 'claude-sonnet-4-6', prompt_revision, '2',
        ...
        response_format => {
            ...
            'schema': {
                ...
                'required': ['assessment', 'issue_type', 'severity', 'surface', 'observation',
                             'evidence_quote', 'suspected_cause', 'preserve_behavior']
            }
        },
