...
WITH raw_review_hashes AS (
    ...
    JOIN raw_review_hashes AS raw ON raw.review_id = findings.review_id
     ...
     AND (findings.validation_status <> 'valid' OR TO_JSON(ARRAY_CONSTRUCT(
         ...
         findings.preserve_behavior, findings.requires_review)) = TO_JSON(ARRAY_CONSTRUCT(
         ...
         raw.raw_review:suspected_cause, raw.raw_review:preserve_behavior, raw.raw_review:requires_review)))
    ...
    WHERE checks.settings_are_valid
      ...
      AND findings.schema_revision = '1'
), latest_saved AS (
...
WITH documentation AS (
    ...
    SELECT 'You propose changes for human review, never apply changes. '
        ...
        || 'Never invent objects, contacts or confirmed causes. If investigate_only is true and warranted, '
        || 'use investigate and provide two distinct actions: data_gap_investigation describes how a person '
...
SELECT inputs.recommendation_id, SYSDATE(), AI_COMPLETE(
