...
--
-- SEARCH_PREVIEW requires constant arguments and returns a VARCHAR JSON string.
-- We wrap it with PARSE_JSON to store an OBJECT in the VARIANT response column.
...
SELECT UUID_STRING(), SYSDATE(), 'instructions.response',
       ...
       PARSE_JSON(SNOWFLAKE.CORTEX.SEARCH_PREVIEW('DOCS_DB.DOCS_SCHEMA.DOCS_SERVICE',
