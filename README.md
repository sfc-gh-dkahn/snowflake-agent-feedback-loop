...
```mermaid
flowchart TD
    capture["02 · Capture<br/>Agent events and settings"]
    rebuild["03 · Rebuild<br/>Turns and follow-up pairs"]
    pair{"Adjacent complete<br/>turns in a thread?"}
    unjudged["Unjudged<br/>No follow-up available"]
    review["04 · Review<br/>Score and save findings"]
    good["Good<br/>Log what worked"]
    unclear["Unclear<br/>No assumption"]
    issues["Poor or data gap<br/>Group by change area"]
    docs["05 · Docs<br/>Retrieve and save passages"]
    gate{"Enough observations<br/>and usable docs?"}
    waiting["Deferred<br/>Below threshold or no docs"]
    recommend["06 · Recommend<br/>Draft and cite docs"]
    inspect["07 · Inspect<br/>Queue, history, errors"]
    human["Human review<br/>No automatic agent changes"]
...
batches. (Note: in a **Snowflake Notebook**, all SQL cells share the same session,
so temporary tables survive across cells naturally).
Submit the complete `DESCRIBE AGENT ->> INSERT ... ;` chain in 02 as one
...
| [01_preflight.sql](sql/01_preflight.sql) | Settings, source agent and events | No writes; privilege probes, readiness, complete-turn counts, and activity profiling |
